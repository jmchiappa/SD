...
static void spiSend(SPIClass *spi_ , uint8_t b) {
  #ifndef USE_SPI_LIB
  SPDR = b;
  while (!(SPSR & (1 << SPIF)))
    ;
...
static  uint8_t spiRec(SPIClass *spi_) {
  #ifndef USE_SPI_LIB
  spiSend( spi_ , 0XFF);
  return SPDR;
  #else
...
//------------------------------------------------------------------------------
/** Soft SPI send */
void spiSend( spi_ , uint8_t data) {
...
uint8_t Sd2Card::cardCommand(uint8_t cmd, uint32_t arg) {
  ...
  // wait for response
  for (uint8_t i = 0; ((status_ = spiRec( spi_ )) & 0X80) && i != 0XFF; i++)
    ;
  return status_;
}
//------------------------------------------------------------------------------
...
uint8_t Sd2Card::readRegister(uint8_t cmd, void* buf) {
  ...
  spiRec( spi_ );  // get first crc byte
  spiRec( spi_ );  // get second crc byte
  chipSelectHigh();
  return true;
...
uint8_t Sd2Card::setSckRate(uint8_t sckRateID) {
  ...
  #else // USE_SPI_LIB
  switch (sckRateID) {
    case 0:  settings = SPISettings(50000000, MSBFIRST, SPI_MODE0); break;
...
uint8_t Sd2Card::waitStartBlock(void) {
  unsigned int t0 = millis();
  while ((status_ = spiRec( spi_ )) == 0XFF) {
    unsigned int d = millis() - t0;
...
uint8_t Sd2Card::writeBlock(uint32_t blockNumber, const uint8_t* src, uint8_t blocking) {
  ...
  if (blocking) {
    ...
    if (cardCommand(CMD13, 0) || spiRec( spi_ )) {
